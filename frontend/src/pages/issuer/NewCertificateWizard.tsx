...
export default function NewCertificateWizard() {
    ...
    const canProceed = (target: StepKey) => {
        switch (target) {
            ...
            case 3:
                return payload.certificateId && payload.studentEmail && payload.studentName && payload.courseName;
            case 4:
    ...
    // Auto-generate canonical + hash when entering review step
    useEffect(() => {
        if (step === 3 && !docHash) {
            computeHash();
        }
                ...
                <div className="stack">
                    ...
                    {step === 0 && (
                        <div className="panel stack">
                            ...
                            <p className="muted">
                                Connect MetaMask with the issuer wallet on Sepolia. Actions are disabled until connected.
                            </p>
                        ...
                        <div className="panel stack">
                            ...
                            <div className="grid two">
                                <input
                                    ...
                                    value={payload.certificateId}
                                    onChange={(e) => setPayload({ ...payload, certificateId: e.target.value })}
                                />
                                <input
                                    className="input"
                                    placeholder="Student email"
                                    value={payload.studentEmail}
                                    onChange={(e) => setPayload({ ...payload, studentEmail: e.target.value })}
                                />
                                <input
                                    className="input"
                                    placeholder="Student name"
                                    value={payload.studentName}
                                    onChange={(e) => setPayload({ ...payload, studentName: e.target.value })}
                                />
                                <input
                                    className="input"
